...
imageData.forEach( elm => {
	...
	for(var i = 1; i <= elm.items; i++){
		var frame = draw.image(elm.path+i+elm.ext,elm.width,elm.height)
					.x(elm.x).y(elm.y).scale(elm.scale.x,elm.scale.y).data('frame',i-1)
	...
	if(elm.macaroni.needed){
		for(var i = 1; i <= elm.items; i++){
			var macaroni = draw.image(mac.path+i+mac.ext,mac.width,mac.height).x(elm.macaroni.x).y(elm.macaroni.y).data('frame',i-1)
